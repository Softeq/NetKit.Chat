...
namespace Softeq.NetKit.Chat.Domain.Services.Channel
{
    ...
    {
        public static ChannelResponse ToChannelResponse(this Domain.Channel.Channel channel,
            ...
            CloudStorageConfiguration configuration)
        {
            ...
            if (channel != null)
            {
                ...
                channelListResponse.LastMessage = channel.Messages?.FirstOrDefault()
                    ?.ToMessageResponse(lastReadMessage, configuration);
                channelListResponse.UnreadMessagesCount = lastReadMessage != null
                    ? channel.Messages?.Count(x => x.Created > lastReadMessage.Created) ?? 0

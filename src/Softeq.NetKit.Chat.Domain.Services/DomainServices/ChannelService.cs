...
namespace Softeq.NetKit.Chat.Domain.Services.DomainServices
{
    internal class ChannelService : BaseService, IChannelService
    {
        ...
        public async Task<ChannelSummaryResponse> CreateChannelAsync(CreateChannelRequest request)
        {
            ...
            if (member == null)
            {
                throw new NetKitChatNotFoundException(NetKitChatNotFoundErrorMessages.UnableCreateChannelCauseNoMember, $"{nameof(request.SaasUserId)}:{request.SaasUserId}");
            }
            ...
            if ((request.Type == ChannelType.Private || request.Type == ChannelType.Direct) && request.AllowedMembers != null && request.AllowedMembers.Any())
            {
                foreach (var memberId in request.AllowedMembers)
                {
                    ...
                    {
                        throw new NetKitChatNotFoundException(NetKitChatNotFoundErrorMessages.UnableAddMember, $"{nameof(memberId)}:{memberId}");
                    }
        ...
        public async Task<IReadOnlyCollection<ChannelResponse>> GetMemberChannelsAsync(string saasUserId)
        {
            ...
            if (member == null)
            {
                throw new NetKitChatNotFoundException(NetKitChatNotFoundErrorMessages.UnableGetMemberChannels, $"{nameof(saasUserId)}:{saasUserId}");
            }
        ...
        public async Task<ChannelResponse> GetChannelByIdAsync(Guid channelId)
        {
            ...
            if (channel == null)
            {
                throw new NetKitChatNotFoundException(NetKitChatNotFoundErrorMessages.UnableGetCahnnel, $"{nameof(channelId)}", $"{nameof(channelId)}:{channelId}");
            }

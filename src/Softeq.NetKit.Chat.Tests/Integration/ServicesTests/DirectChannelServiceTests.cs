...
namespace Softeq.NetKit.Chat.Tests.Integration.ServicesTests
{
    public class DirectChannelServiceTests : BaseTest
    {
        ...
        public async Task GetMessagesByChannelIdAsyncTest()
        {
            ...
            // Assert
            Assert.Equal(4, directMessagesResponse.Count);
            Assert.Equal(3, directMessagesResponse.Count(x => x.Owner.Id == ownerId));
            Assert.Equal(1, directMessagesResponse.Count(x => x.Owner.Id == memberId));
        }
    }
}

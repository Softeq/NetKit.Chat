...
namespace Softeq.NetKit.Chat.Tests.Unit.Domain.Services.MessageServiceTests
{
    public class GetMessagesAsyncTests : MessageServiceTestBase
    {
        ...
        public void ShouldThrowIfMemberDoesNotExist()
        {
            ...
            // Act
            Func<Task> act = async () => { await _messageService.GetMessagesAsync(request); };

            // Assert
            act.Should().Throw<NetKitChatNotFoundException>()
                .And.Message.Should().Be($"Unable to get messages. Member {nameof(request.SaasUserId)}:{request.SaasUserId} is not found.");
        ...
        [Fact]
        [Trait("Category", "Unit")]
        public async Task ShouldReturnMessages_WhenFoundLastMessageByMessageId_AndFoundLastReadMessage()
        {
            // Arrange
            ...
            _messageRepositoryMock.Setup(x => x.GetMessagesWithOwnersAsync(
                    It.Is<Guid>(channelId => channelId.Equals(request.ChannelId)),
        ...
        [Fact]
        [Trait("Category", "Unit")]
        public async Task ShouldReturnMessages_WhenLastMessageByMessageIdNotFound_AndLastReadMessageNotFound()
        {
            // Arrange
            var request = new GetMessagesRequest("864EB62D-D833-47FA-8A88-DDBFE76AE6A7", new Guid("A455F139-09E6-4EF5-B55A-D4C94D05DFDE"), new Guid("01ADF1C2-0B38-4E4F-9E36-0C0BC7908062"), DateTimeOffset.Now, 10);
